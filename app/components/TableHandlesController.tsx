...
export const TableHandlesController: FC<TableHandlesControllerProps> = ({
    ...
    const handleRemoveLastRow = () => {
        if (!editor) return;
        const tableEl = state?.tableElement;
        if (!tableEl) return;

        const lastRow = tableEl.querySelector("tr:last-child");
        if (!lastRow) return;

        const lastCell = lastRow.querySelector("td:first-child, th:first-child");
        if (!lastCell) return;

        const cellDesc = (lastCell as any).pmViewDesc;
        if (!cellDesc) {
            console.warn("No pmViewDesc found on lastCell");
            return;
        }
        if (
            cellDesc.node.type.name !== "tableCell" &&
            cellDesc.node.type.name !== "tableHeader"
        ) {
            console.error("Resolved node is not a tableCell:", cellDesc.node.type.name);
            return;
        }

        const cellPos = cellDesc.posBefore;
        const $cellPos = editor.state.doc.resolve(cellPos);

        const cellSelection = CellSelection.create(editor.state.doc, $cellPos.pos);
    ...
    const handleRemoveLastColumn = () => {
        ...
        if (cellDesc.node.type.name !== "tableCell" && cellDesc.node.type.name !== "tableHeader") {
            console.error("Resolved node is not a tableCell:", cellDesc.node.type.name);
            return;
        }
        const cellPos = cellDesc.posBefore;
