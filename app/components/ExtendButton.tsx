...
// Styled components
const ExtendButtonContainer = styled.button<{ $isRow: boolean; $isDragging: boolean }>`
...
const ExtendButton: FC<ExtendButtonProps> = ({ orientation, onClick, editor, tableElement }) => {
    ...
    const handleMouseDown = (e: React.MouseEvent) => {
        ...
        const handleMouseMove = (e: MouseEvent) => {
            ...
            if (operationsNeeded !== 0) {
                ...
                try {
                    if (operationsNeeded > 0) {
                        ...
                        for (let i = 0; i < operationsNeeded; i++) {
                            if (isRow) {
                                ...
                                const lastCell = tableElement?.querySelector(`tbody tr:nth-child(${rows}) td:first-child, tbody tr:nth-child(${rows}) th:first-child`);
                                if (lastCell) {
                    ...
                    console.log(`Applied ${Math.abs(operationsNeeded)} ${operationsNeeded > 0 ? 'additions' : 'removals'}. Total applied: ${appliedChangesRef.current}`);
        ...
        const handleMouseUp = () => {
            console.log(`${isRow ? 'Row' : 'Column'} drag ended. Total changes applied: ${appliedChangesRef.current}`);
