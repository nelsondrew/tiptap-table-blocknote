...
const TableCellContainer = styled.div<{ editMode?: boolean; hasFlexChart?: boolean }>`
  border: ${props =>
...
export const EditorCell: React.FC<EditorCellProps> = ({
  ...
  const editor = useEditor({
    ...
    editable: editMode && !hasFlexChart,
    editorProps: {
      attributes: {
        id: editorId,
        class: 'three-cols-editor'
      },
        ...
        if (!updateScheduled.current) {
          ...
          requestAnimationFrame(() => {
            ...
            requestAnimationFrame(() => {
              ...
              requestAnimationFrame(() => {
                editor.commands.setTextSelection({ from, to : to + 5 });
  ...
  // Update editable state when editMode or content changes
  useEffect(() => {
    if (editor) {
      editor.setEditable(editMode && !hasFlexChart);
    }
  }, [editor, editMode, hasFlexChart]);
