    ...
    addNodeView() {
        return ({ node, editor }) => {
          ...
          `;
            ...
            let tableId = null;
            let parentTable = wrapper.parentElement;
            ...
            const scrollAuthorityObserver = new MutationObserver((mutations) => {
                mutations.forEach((mutation) => {
                    if (mutation.attributeName === 'class') {
                        ...
                        if (currentTableId && editor.tableScrollAuthority) {
                            if (hasShowScrollbar) {
                                // Register as candidate for scroll authority
                                editor.tableScrollAuthority.registerCandidate(currentTableId, rowId);
                            }
                            else {
                                // Remove from candidates
                                editor.tableScrollAuthority.removeCandidate(currentTableId, rowId);
                            }
                        }
                    }
                });
            ...
            const calculateColumnWidths = (currentNode) => {
                const columnWidths = [];
                currentNode.forEach((cell) => {
                ...
                ignoreMutation(mutation) {
                    ...
                    if (mutation.type === 'attributes') {
                        ...
                        if (attributeName === 'style' ||
                            attributeName === 'class' ||
                            (attributeName === null || attributeName === void 0 ? void 0 : attributeName.startsWith('data-'))) {
                            return true;
                        }
                    }
